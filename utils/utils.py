...
def overlay_masks(image, masks, labels, colors, mask_alpha):
    # 颜色列表
    color = [value for key, value in colors.items()]
   
    expanded_image = np.expand_dims(image, axis=2)
    expanded_image = np.repeat(expanded_image, 3, axis=2)
    ...
    # 创建叠加图像
    overlay_image = np.dstack((overlay, alpha))
...
def test_single_volume(image, label, net, classes, patch_size=[256, 256], test_save_path=None, case=None, z_spacing=1, class_names=None):
    ...
    my_colors=['red','yellow','forestgreen','blue','magenta','cyan','deepskyblue','darkviolet']
    cmap = {k: cmaps[k] for k in sorted(cmaps.keys()) if k in my_colors[:classes-1]}
    if len(image.shape) == 3:
        prediction = np.zeros_like(label)
        for ind in range(image.shape[0]):  # 整个切片（3维数据） 148 x 512 x 512
            slice = image[ind, :, :]       # 2维数据
            x, y = slice.shape[0], slice.shape[1] # H，W
            ...
            with torch.no_grad():
                ...
                lbl = label[ind, :, :]  # 2维
                masks = []
                for i in range(1, classes):  # 标签
                    masks.append(lbl==i)
                preds_o = []
                for i in range(1, classes):  # 预测
                ...
                # plt.imshow(fig_gt)
                # plt.axis('off')
                # plt.savefig(test_save_path + '/'+case + '_' +str(ind)+'_gt.png', bbox_inches="tight", dpi=300)
                # plt.close()
                # plt.imshow(fig_pred)
                # plt.axis('off')
                # plt.savefig(test_save_path + '/'+case + '_' +str(ind)+'_pred.png', bbox_inches="tight", dpi=300)
                # plt.close()
