...
from lib.networks import MERIT_GLAG_Cascaded
...
# wo use the model           
net = MERIT_GLAG_Cascaded(n_class=args.num_classes, img_size_s1=(args.img_size,args.img_size), img_size_s2=(224,224), model_scale='small', decoder_aggregation='additive', interpolation='bilinear').cuda()
...
for epoch in iterator:
    ...
    if avg_test_dcs > Best_dcs:
        Best_dcs = avg_test_dcs
        print("test avg_dsc: %f Best_dcs: %f" % (avg_test_dcs, Best_dcs))
        logging.info("test avg_dsc: %f Best_dcs: %f" % (avg_test_dcs, Best_dcs))
        save_model_path = os.path.join(snapshot_path, 'best.pth')
        torch.save(net.state_dict(), save_model_path)
        logging.info("save model to {}".format(save_model_path))
        print("save model to {}".format(save_model_path))
    ...
    if epoch >= args.max_epochs - 1:
        save_model_path = os.path.join(snapshot_path, 'epoch={}_lr={}_avg_dcs={}.pth'.format(epoch, lr_, avg_test_dcs))
        torch.save(net.state_dict(), save_model_path)
        logging.info("save model to {}".format(save_model_path))
        print("save model to {}".format(save_model_path))
        iterator.close()
